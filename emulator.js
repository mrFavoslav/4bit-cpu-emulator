class CPU {
  ...
  attachEventListeners() {
    ...
    applyButton.addEventListener("click", () => {
      ...
      // Wait 100ms before running again (only if not halted)
    ...
    lines.forEach((line) => {
      ...
      if (line) {
        try {
          ...
          console.error(
            `Error assembling instruction "${line}": ${error.message}`,
          );
        }
      }
    });
  ...
  assemble(instruction, firstPass = false) {
    const opcodes = {
      ...
      CMP: 0x1c, // Compare
      INT: 0xfe, // Interrupt (BIOS-style services)
      HLT: 0xff, // Halt (see README)
    };
    ...
    if (parts[0] === "INT") {
      if (operands.length !== 1) {
        throw new Error(
          `Invalid number of operands for INT instruction: ${instruction}`,
        );
      }
    ...
    if (
      ["NOT", "JMP", "JZ", "JC"].includes(parts[0]) &&
      operands.length === 1
    ) {
      if (operands[0].startsWith("#")) {
        instrType = 0x03;
  ...
  step() {
    ...
    console.log(
      `Executing instruction at ${new Date().toTimeString().split(" ")[0]}.${new Date().getMilliseconds().toString().padStart(3, "0")}: opcode=${opcode.toString(16)}, type=${type.toString(16)}, PC=${this.registers.PC.toString(16)}`,
    );
  ...
  getRegisterValue(registerCode) {
    ...
    if (this.registers.F.MOP === 1) {
      ...
      if (highLow === "high") {
        ...
        return (
          ((this.registers[register][0] & 0xf) << 4) |
          (this.registers[register][1] & 0xf)
        );
      }
    } else {
      // 8-bit mode - still using two 4-bit banks
      if (highLow === "high") {
        return this.registers[register][0] & 0xf;
      } else if (highLow === "low") {
        return this.registers[register][1] & 0xf;
  ...
  execute(opcode, type) {
    try {
      ...
      console.error(
        `Error executing instruction ${opcode.toString(16)}: ${error.message}`,
      );
      this.running = false;
    }
  }
  ...
  executeINT(type) {
    switch (type) {
      case 0x00: {
        // INT reg - Interrupt using register value
      ...
      case 0x01: {
        // INT [mem] - Interrupt using memory value
      ...
      case 0x03: {
        // INT #immediate - Interrupt using immediate value
  ...
  handleInterrupt(intNum) {
    ...
    switch (intNum) {
      ...
      case 1: // INT 1 - Output character from AX
        console.log(
          `INT 1: Char = ${String.fromCharCode(this.getRegisterValue(0x01))}`,
        );
        break;
      default:
  ...
  executeMOV(type) {
    switch (type) {
      case 0x00: {
        // MOV reg, reg - Move from register to register
      ...
      case 0x03: {
        // MOV reg, #immediate - Move immediate value to register
        const destReg = this.memory[this.registers.PC++];
      ...
      case 0x04: {
        // MOV MOP, #immediate - Set Memory Operation Mode
        const mode = this.memory[this.registers.PC++];
        this.registers.F.MOP = mode === 0 ? 0 : 1;
        console.log(
          `MOP mode set to ${this.registers.F.MOP === 1 ? "4-bit" : "8-bit"}`,
        );
        break;
      }
      case 0x05: {
        // MOV [mem1], [mem2] - Move from memory to memory
        const destAddress = this.memory[this.registers.PC++];
        const srcAddress = this.memory[this.registers.PC++];
        if (
          destAddress < this.memory.length &&
          srcAddress < this.memory.length
        ) {
          const value = this.memory[srcAddress];
          this.memory[destAddress] = value;
        } else {
          console.error(
            `Memory access out of bounds: dest=${destAddress}, src=${srcAddress}`,
          );
        }
        break;
      }
      case 0x06: {
        // MOV [mem], #immediate - Move immediate value to memory
        const address = this.memory[this.registers.PC++];
        const value = this.memory[this.registers.PC++];
        if (address < this.memory.length) {
          this.memory[address] = value & 0xff;
  ...
  executeADD(type) {
    switch (type) {
      ...
      case 0x01: {
        // ADD reg, [mem] - Add memory to register
        const destReg = this.memory[this.registers.PC++];
        const address = this.memory[this.registers.PC++];
        if (address < this.memory.length) {
          const a = this.getRegisterValue(destReg);
      ...
      case 0x02: {
        // ADD [mem], reg - Add register to memory
        const address = this.memory[this.registers.PC++];
        const srcReg = this.memory[this.registers.PC++];
        if (address < this.memory.length) {
      ...
      case 0x03: {
        // ADD reg, #immediate - Add immediate to register
        const destReg = this.memory[this.registers.PC++];
        const value = this.memory[this.registers.PC++];
        const a = this.getRegisterValue(destReg);
      ...
      case 0x05: {
        // ADD [mem1], [mem2] - Add memory to memory
        const destAddress = this.memory[this.registers.PC++];
        const srcAddress = this.memory[this.registers.PC++];
        if (
          destAddress < this.memory.length &&
      ...
      case 0x06: {
        // ADD [mem], #immediate - Add immediate to memory
        const address = this.memory[this.registers.PC++];
        const value = this.memory[this.registers.PC++];
        if (address < this.memory.length) {
          const a = this.memory[address];
  ...
  executeSUB(type) {
    switch (type) {
      ...
      case 0x01: {
        // SUB reg, [mem] - Subtract memory from register
        const destReg = this.memory[this.registers.PC++];
        const address = this.memory[this.registers.PC++];
        if (address < this.memory.length) {
          const a = this.getRegisterValue(destReg);
      ...
      case 0x02: {
        // SUB [mem], reg - Subtract register from memory
        const address = this.memory[this.registers.PC++];
        const srcReg = this.memory[this.registers.PC++];
        if (address < this.memory.length) {
          const a = this.memory[address];
      ...
      case 0x03: {
        // SUB reg, #immediate - Subtract immediate from register
        const destReg = this.memory[this.registers.PC++];
        const value = this.memory[this.registers.PC++];
        const a = this.getRegisterValue(destReg);
        const result = a - value;
      ...
      case 0x05: {
        // SUB [mem1], [mem2] - Subtract memory from memory
        const destAddress = this.memory[this.registers.PC++];
        const srcAddress = this.memory[this.registers.PC++];
        if (
          destAddress < this.memory.length &&
      ...
      case 0x06: {
        // SUB [mem], #immediate - Subtract immediate from memory
        const address = this.memory[this.registers.PC++];
        const value = this.memory[this.registers.PC++];
        if (address < this.memory.length) {
          const a = this.memory[address];
  ...
  executeAND(type) {
    switch (type) {
      case 0x00: {
        // AND reg, reg - Perform logical AND between registers
        const destReg = this.memory[this.registers.PC++];
        const srcReg = this.memory[this.registers.PC++];
        const a = this.getRegisterValue(destReg);
        const b = this.getRegisterValue(srcReg);
      ...
      case 0x01: {
        // AND reg, [mem] - Perform logical AND between register and memory
        const destReg = this.memory[this.registers.PC++];
        const address = this.memory[this.registers.PC++];
        if (address < this.memory.length) {
          const a = this.getRegisterValue(destReg);
      ...
      case 0x02: {
        // AND [mem], reg - Perform logical AND between memory and register
        const address = this.memory[this.registers.PC++];
        const srcReg = this.memory[this.registers.PC++];
        if (address < this.memory.length) {
          const a = this.memory[address];
      ...
      case 0x03: {
        // AND reg, #immediate - Perform logical AND between register and immediate value
        const destReg = this.memory[this.registers.PC++];
        const value = this.memory[this.registers.PC++];
        const a = this.getRegisterValue(destReg);
        const result = a & value;
      ...
      case 0x05: {
        // AND [mem1], [mem2] - Perform logical AND between two memory locations
        const destAddress = this.memory[this.registers.PC++];
        const srcAddress = this.memory[this.registers.PC++];
        if (
          destAddress < this.memory.length &&
      ...
      case 0x06: {
        // AND [mem], #immediate - Perform logical AND between memory and immediate value
        const address = this.memory[this.registers.PC++];
        const value = this.memory[this.registers.PC++];
        if (address < this.memory.length) {
          const a = this.memory[address];
  ...
  executeOR(type) {
    switch (type) {
      case 0x00: {
        // OR reg, reg - Perform logical OR between registers
        const destReg = this.memory[this.registers.PC++];
        const srcReg = this.memory[this.registers.PC++];
        const a = this.getRegisterValue(destReg);
        const b = this.getRegisterValue(srcReg);
      ...
      case 0x01: {
        // OR reg, [mem] - Perform logical OR between register and memory
        const destReg = this.memory[this.registers.PC++];
        const address = this.memory[this.registers.PC++];
        if (address < this.memory.length) {
          const a = this.getRegisterValue(destReg);
      ...
      case 0x02: {
        // OR [mem], reg - Perform logical OR between memory and register
        const address = this.memory[this.registers.PC++];
        const srcReg = this.memory[this.registers.PC++];
        if (address < this.memory.length) {
          const a = this.memory[address];
      ...
      case 0x03: {
        // OR reg, #immediate - Perform logical OR between register and immediate value
        const destReg = this.memory[this.registers.PC++];
        const value = this.memory[this.registers.PC++];
        const a = this.getRegisterValue(destReg);
        const result = a | value;
      ...
      case 0x05: {
        // OR [mem1], [mem2] - Perform logical OR between two memory locations
        const destAddress = this.memory[this.registers.PC++];
        const srcAddress = this.memory[this.registers.PC++];
        if (
          destAddress < this.memory.length &&
      ...
      case 0x06: {
        // OR [mem], #immediate - Perform logical OR between memory and immediate value
        const address = this.memory[this.registers.PC++];
        const value = this.memory[this.registers.PC++];
        if (address < this.memory.length) {
          const a = this.memory[address];
  ...
  executeXOR(type) {
    switch (type) {
      case 0x00: {
        // XOR reg, reg - Perform logical XOR between registers
        const destReg = this.memory[this.registers.PC++];
        const srcReg = this.memory[this.registers.PC++];
        const a = this.getRegisterValue(destReg);
        const b = this.getRegisterValue(srcReg);
      ...
      case 0x01: {
        // XOR reg, [mem] - Perform logical XOR between register and memory
        const destReg = this.memory[this.registers.PC++];
        const address = this.memory[this.registers.PC++];
        if (address < this.memory.length) {
          const a = this.getRegisterValue(destReg);
      ...
      case 0x02: {
        // XOR [mem], reg - Perform logical XOR between memory and register
        const address = this.memory[this.registers.PC++];
        const srcReg = this.memory[this.registers.PC++];
        if (address < this.memory.length) {
          const a = this.memory[address];
      ...
      case 0x03: {
        // XOR reg, #immediate - Perform logical XOR between register and immediate value
        const destReg = this.memory[this.registers.PC++];
        const value = this.memory[this.registers.PC++];
        const a = this.getRegisterValue(destReg);
        const result = a ^ value;
      ...
      case 0x05: {
        // XOR [mem1], [mem2] - Perform logical XOR between two memory locations
        const destAddress = this.memory[this.registers.PC++];
        const srcAddress = this.memory[this.registers.PC++];
        if (
          destAddress < this.memory.length &&
      ...
      case 0x06: {
        // XOR [mem], #immediate - Perform logical XOR between memory and immediate value
        const address = this.memory[this.registers.PC++];
        const value = this.memory[this.registers.PC++];
        if (address < this.memory.length) {
          const a = this.memory[address];
  ...
  executeNOT(type) {
    switch (type) {
      case 0x00: {
        // NOT reg - Perform logical NOT on register
        const destReg = this.memory[this.registers.PC++];
        const a = this.getRegisterValue(destReg);
        const result = ~a & 0xff;
        this.setRegisterValue(destReg, result);
      ...
      case 0x01: {
        // NOT [mem] - Perform logical NOT on memory
        const address = this.memory[this.registers.PC++];
        if (address < this.memory.length) {
          const a = this.memory[address];
          const result = ~a & 0xff;
  ...
  executeJMP(type) {
    switch (type) {
      ...
      case 0x03: {
        // JMP #immediate - Jump to immediate address
        const address = this.memory[this.registers.PC++];
        if (address < this.memory.length) {
          this.registers.PC = address;
        } else {
  ...
  executeJZ(type) {
    if (this.registers.F.ZF === 1) {
      switch (type) {
        case 0x00: {
          // JZ reg - Jump to address in register if zero flag is set
          const srcReg = this.memory[this.registers.PC++];
          const address = this.getRegisterValue(srcReg);
          if (address < this.memory.length) {
            this.registers.PC = address;
        ...
        case 0x01: {
          // JZ [mem] - Jump to address in memory if zero flag is set
          const addressPtr = this.memory[this.registers.PC++];
          if (addressPtr < this.memory.length) {
            const address = this.memory[addressPtr];
            if (address < this.memory.length) {
        ...
        case 0x03: {
          // JZ #immediate - Jump to immediate address if zero flag is set
          const address = this.memory[this.registers.PC++];
          if (address < this.memory.length) {
            this.registers.PC = address;
          } else {
  ...
  executeJC(type) {
    if (this.registers.F.CF === 1) {
      switch (type) {
        case 0x00: {
          // JC reg - Jump to address in register if carry flag is set
          const srcReg = this.memory[this.registers.PC++];
          const address = this.getRegisterValue(srcReg);
          if (address < this.memory.length) {
            this.registers.PC = address;
        ...
        case 0x01: {
          // JC [mem] - Jump to address in memory if carry flag is set
          const addressPtr = this.memory[this.registers.PC++];
          if (addressPtr < this.memory.length) {
            const address = this.memory[addressPtr];
            if (address < this.memory.length) {
        ...
        case 0x03: {
          // JC #immediate - Jump to immediate address if carry flag is set
          const address = this.memory[this.registers.PC++];
          if (address < this.memory.length) {
            this.registers.PC = address;
          } else {
  ...
  executeSHL(type) {
    switch (type) {
      case 0x00: {
        // SHL reg, reg - Shift register left by count in another register
        const destReg = this.memory[this.registers.PC++];
        const srcReg = this.memory[this.registers.PC++];
        const a = this.getRegisterValue(destReg);
      ...
      case 0x01: {
        // SHL reg, [mem] - Shift register left by count in memory
        const destReg = this.memory[this.registers.PC++];
        const address = this.memory[this.registers.PC++];
        if (address < this.memory.length) {
          const a = this.getRegisterValue(destReg);
      ...
      case 0x02: {
        // SHL [mem], reg - Shift memory left by count in register
        const address = this.memory[this.registers.PC++];
        const srcReg = this.memory[this.registers.PC++];
        if (address < this.memory.length) {
          const a = this.memory[address];
      ...
      case 0x03: {
        // SHL reg, #immediate - Shift register left by immediate count
        const destReg = this.memory[this.registers.PC++];
      ...
      case 0x05: {
        // SHL [mem1], [mem2] - Shift memory left by count in another memory location
        const destAddress = this.memory[this.registers.PC++];
        const srcAddress = this.memory[this.registers.PC++];
        if (
          destAddress < this.memory.length &&
      ...
      case 0x06: {
        // SHL [mem], #immediate - Shift memory left by immediate count
        const address = this.memory[this.registers.PC++];
        const count = this.memory[this.registers.PC++] & 0x7; // Max 7 bits shift
        if (address < this.memory.length) {
          const a = this.memory[address];
  ...
  executeSHR(type) {
    switch (type) {
      case 0x00: {
        // SHR reg, reg - Shift register right by count in another register
        const destReg = this.memory[this.registers.PC++];
        const srcReg = this.memory[this.registers.PC++];
        const a = this.getRegisterValue(destReg);
        const count = this.getRegisterValue(srcReg) & 0x7; // Max 7 bits shift
      ...
      case 0x01: {
        // SHR reg, [mem] - Shift register right by count in memory
        const destReg = this.memory[this.registers.PC++];
        const address = this.memory[this.registers.PC++];
        if (address < this.memory.length) {
          const a = this.getRegisterValue(destReg);
      ...
      case 0x02: {
        // SHR [mem], reg - Shift memory right by count in register
        const address = this.memory[this.registers.PC++];
        const srcReg = this.memory[this.registers.PC++];
        if (address < this.memory.length) {
          const a = this.memory[address];
      ...
      case 0x03: {
        // SHR reg, #immediate - Shift register right by immediate count
        const destReg = this.memory[this.registers.PC++];
        const count = this.memory[this.registers.PC++] & 0x7; // Max 7 bits shift
        const a = this.getRegisterValue(destReg);
        const result = a >> count;
      ...
      case 0x05: {
        // SHR [mem1], [mem2] - Shift memory right by count in another memory location
        const destAddress = this.memory[this.registers.PC++];
        const srcAddress = this.memory[this.registers.PC++];
        if (
          destAddress < this.memory.length &&
      ...
      case 0x06: {
        // SHR [mem], #immediate - Shift memory right by immediate count
        const address = this.memory[this.registers.PC++];
        const count = this.memory[this.registers.PC++] & 0x7; // Max 7 bits shift
        if (address < this.memory.length) {
          const a = this.memory[address];
  ...
  executeCMP(type) {
    switch (type) {
      ...
      case 0x01: {
        // CMP reg, [mem] - Compare register with memory
        const destReg = this.memory[this.registers.PC++];
        const address = this.memory[this.registers.PC++];
        if (address < this.memory.length) {
          const a = this.getRegisterValue(destReg);
      ...
      case 0x02: {
        // CMP [mem], reg - Compare memory with register
        const address = this.memory[this.registers.PC++];
        const srcReg = this.memory[this.registers.PC++];
        if (address < this.memory.length) {
          const a = this.memory[address];
      ...
      case 0x03: {
        // CMP reg, #immediate - Compare register with immediate value
        const destReg = this.memory[this.registers.PC++];
        const value = this.memory[this.registers.PC++];
        const a = this.getRegisterValue(destReg);
        const result = a - value;
      ...
      case 0x05: {
        // CMP [mem1], [mem2] - Compare memory with memory
        const destAddress = this.memory[this.registers.PC++];
        const srcAddress = this.memory[this.registers.PC++];
        if (
          destAddress < this.memory.length &&
      ...
      case 0x06: {
        // CMP [mem], #immediate - Compare memory with immediate value
        const address = this.memory[this.registers.PC++];
        const value = this.memory[this.registers.PC++];
        if (address < this.memory.length) {
          const a = this.memory[address];
  ...
  updateFlagsFromResult(result) {
    ...
    this.registers.F.SF =
      (maskedResult & (this.registers.F.MOP === 1 ? 0x8 : 0x80)) !== 0 ? 1 : 0;
  ...
  updateMemoryDisplay() {
    ...
    for (let row = 0; row < 16; row++) {
      ...
      for (let col = 0; col < 16; col++) {
        ...
        td.textContent = this.memory[address]
          .toString(16)
          .padStart(2, "0")
          .toUpperCase();
